...
import { CODEX_MODELS, TOOLS_DIR } from '@shared/constants'
...
export class CodexDriver implements AgentDriver {
    ...
    return new Promise((resolve, reject) => {
      ...
      proc.onExit(({ exitCode }) => {
        if (exitCode === 0) {
          ...
          reject(new Error(`Codex installation failed with exit code ${exitCode}`))
        }
      })
    })
  }
  ...
  async getVersion(): Promise<string | null> {
    ...
    const pkgPath = join(TOOLS_DIR, 'node_modules', '@openai', 'codex', 'package.json')
  ...
  async testConnection(apiKey: string): Promise<boolean> {
    const binPath = this.resolveBinaryPath()
    if (!binPath || !apiKey) return false

    const result = spawnSync(binPath, ['--version'], {
      encoding: 'utf-8',
      env: this.cleanEnv({ OPENAI_API_KEY: apiKey })
    })
  ...
  private localBinaryPath(): string {
    return join(TOOLS_DIR, 'node_modules', '.bin', process.platform === 'win32' ? 'codex.cmd' : 'codex')
  }
  ...
  private resolveBinaryPath(): string | null {
    ...
    const result = spawnSync(lookupCmd, ['codex'], { encoding: 'utf-8' })
  ...
  private cleanEnv(extra: Record<string, string> = {}): Record<string, string> {
    ...
    // Prevent nested Codex session hints from parent shells.
    delete env['CODEX_SESSION_ID']
    delete env['CODEX_SANDBOX_MODE']
    return env
  }
  ...
  private wrapPty(pty: ReturnType<typeof ptySpawn>): AgentProcess {
    ...
    const sessionIdRegex = /(?:Session(?: ID)?|session[_\s-]?id|conversation[_\s-]?id)[:\s]+([a-f0-9-]{32,})/i
    let parsed = false
